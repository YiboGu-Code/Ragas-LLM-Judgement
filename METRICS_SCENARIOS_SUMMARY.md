...
- Ragas 的核心思路不是“直接让 LLM 打一个总分”，而是先做“反向问题生成”：
  - 从 `record.input` 提取 `user_input`
  - 从 `trace.output` 提取 `answer`
  - 让 LLM 根据 `answer` 反向生成 `N` 个可能对应的问题，记为 `q'_1, q'_2, ..., q'_N`
  - 对原问题 `q` 和每个反向问题 `q'_i` 计算 embedding
- 近似计算公式：

  \[
  \text{AnswerRelevancy}(q, a) = \frac{1}{N} \sum\_{i=1}^{N} \cos\left(E(q'\_i), E(q)\right)
  \]
...
- 本项目实现：
  ...
  - 通过 `aevaluate()` 运行，最终返回 `0~1` 左右的连续分值
...
- 第二步：基于 statement 级别的 TP/FP/FN 计算事实一致性的 F1：
  ...
  \[
  F\_{\text{fact}} = \frac{|TP|}{|TP| + 0.5 \times (|FP| + |FN|)}
  \]

- 第三步：计算回答与参考答案的语义相似度：

  \[
  S\_{\text{sem}} = \cos\left(E(a), E(r)\right)
  \]
...
- 第四步：加权合成最终分数。Ragas 经典实现中常见默认权重为 factuality `0.75`、semantic similarity `0.25`：
  ...
  ```text
  AnswerCorrectness = 0.75 * Ffact + 0.25 * Ssem
  ```

- 本项目实现差异：
  - 如果 `reference.strip() == answer.strip()`，直接走快捷路径返回 `1.0`
...
- 作用：评估回答是否忠实于检索到的上下文。
- **Ragas 原始思路：**
  - 先把 `answer` 拆成若干 statement：`s_1, s_2, ..., s_m`
  - 对每个 statement，用 LLM 判断它是否能被 `retrieved_contexts` 支撑
  - 最终分数是“可被上下文支撑的 statement 比例”：

  \[
  \text{Faithfulness}_{\text{ragas}} = \frac{\sum_{j=1}^{m} \mathbf{1}[s_j \text{ 可由上下文推出}]}{m}
  \]

- **本项目当前实现并没有直接走上述 LLM 判定链路**，而是改成了启发式 `shingle overlap`，公式为：
  - 先将 `answer` 归一化后切成 `n`-shingles，记为 `S_n(answer)`
  - 将所有 contexts 拼接、归一化后切成 `n`-shingles，记为 `S_n(contexts)`
  - 当前分数：

  \[
  \text{Faithfulness}\_{\text{project}} =
  \frac{|S_n(answer) \cap S_n(contexts)|}{|S_n(answer)|}
  \]

- 代码中的具体实现约束：
  - `n = RAGAS_HEURISTIC_SHINGLE_N`，默认 `2`
...
- 仍采用“基于回答反向生成 `N` 个问题，再与原问题做 embedding 相似度平均”的方式：
...
- 则第 `k` 位的 precision 为：
  ...
  \[
  P@k = \frac{\sum\_{i=1}^{k} v_i}{k}
  \]

- 最终 Context Precision 为：

  ```text
  ContextPrecision@K = (sum_{k=1..K} (P@k * v_k)) / (sum_{k=1..K} v_k)
  ```
...
- 这个公式的含义是：
  ...
  - 所以它是一个“排序敏感”的检索质量指标。
- 本项目实现：
  - 提供 `{ user_input, reference, retrieved_contexts }`
  - 调用 `ContextPrecision()`
...
- **Ragas 原始思路：**
  - 先把 `reference` 拆成 statement：`r_1, r_2, ..., r_t`
  - 对每个 statement，判断是否能被 `retrieved_contexts` 支撑
  - 最终分数为：

  \[
  \text{ContextRecall}_{\text{ragas}} =
  \frac{\sum_{j=1}^{t} \mathbf{1}[r_j \text{ 可由上下文推出}]}{t}
  \]
...
- **本项目当前实现是启发式近似：**
  ...
  \[
  \text{ContextRecall}\_{\text{project}} =
  \frac{|S_n(reference) \cap S_n(contexts)|}{|S_n(reference)|}
  \]
...
- 计算公式与 Prompt 场景一致：
  ...
  ```text
  Ffact = |TP| / (|TP| + 0.5 * (|FP| + |FN|))
  Ssem = cosine(E(answer), E(reference))
  AnswerCorrectness = 0.75 * Ffact + 0.25 * Ssem
  ```
...
- 对于 `WithoutReference` 版本，Ragas 的思路是：
  - 从整段多轮对话 / tool use 轨迹中推断用户目标 `g`
  - 再让 LLM 判断最终状态 `T` 是否满足目标 `g`
- 可以把它抽象写成：
...
- 本项目实现：
  ...
  - 若缺少完整多轮消息，则退化为 `[HumanMessage(user_input), AIMessage(answer)]`
  - 组装 `MultiTurnSample`
...
- 当前项目中启发式分数最核心的统一模式为：
  ...
  \[
  \text{Overlap}(A, B) = \frac{|S_n(A) \cap S_n(B)|}{|S_n(A)|}
  \]

  其中 `S_n(\cdot)` 表示归一化文本后的 `n`-shingle 集合。

- 所以：
  - `ragas_faithfulness` 近似看的是 `answer` 被 `contexts` 覆盖的比例
  - `ragas_context_recall` 近似看的是 `reference` 被 `contexts` 覆盖的比例
- 在答辩时，建议明确说：
  - `answer_relevancy` / `answer_correctness` / `context_precision` / `agent_goal_accuracy` 仍主要遵循 Ragas 的 LLM/embedding 判定链路
  - `faithfulness` / `context_recall` 在本项目里为了成本和稳定性，替换成了可解释的启发式公式
